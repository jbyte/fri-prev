...
Semantic function
$$ \valattr{\cdot} \colon {\cal P} \rightarrow \{(-2^{63})\ldots(+2^{63}-1)\} $$
maps phrases of PREV to the integer values they denote.  It is defined by the following rules:
%
$$ \begin{array}{c}
   -2^{63} \leq \mbox{\textit{lexeme}}(\term{INTEGER}) \leq +2^{63}-1
   \\[2pt]\hline\\[-10pt]
...
$$ \valattr{\cdot} \colon {\cal P} \rightarrow \mathrm{T} $$
